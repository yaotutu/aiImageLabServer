...
export class UsersController {
  ...
  @ApiOperation({ summary: '修改密码（兼容接口）', description: '修改密码的兼容接口，与 PUT /api/users/password 功能相同' })
  @ApiBody({
    schema: {
      type: 'object',
      required: ['oldPassword', 'newPassword'],
  ...
  @ApiResponse({ status: 200, description: '密码修改成功' })
  async changePasswordCompat(@Req() req: any, @Body() body: { oldPassword: string; newPassword: string }) {
    await this.usersService.changePassword(req.user.userId, body.oldPassword, body.newPassword);
    return { message: '密码修改成功' };
  }
  ...
  @ApiOperation({ summary: '获取用户生成统计', description: '获取用户的生成统计信息，包括成功率、消耗积分等' })
  @ApiResponse({ status: 200, description: '成功' })
